...
$JarPath = Join-Path $RuntimeRoot 'TheMechanistServer.jar'
...
Write-LogLine "The Mechanist server launcher started at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')"
...
Write-Host 'Launching The Mechanist server...'
$output = & $java.Source -Xms256m -Xmx2048m -jar $JarPath @ServerArgs 2>&1
$exitCode = $LASTEXITCODE
foreach ($line in $output) { Write-LogLine ([string] $line); Write-Host ([string] $line) }
