...
- `MILESTONE_00_GOVERNANCE_PACKAGE_AND_INDEX.md` - governance, package, milestone reading order, anti-sprawl, editor-suite routing, legacy source-map routing, and Codex handoff rules.
...
- `MILESTONE_04_POPULATION_PROVENANCE_ECONOMY_AND_FACTION_MARKETS.md` - population provenance, item provenance, raw-material provenance, deferred sector simulation, reinforcement imports, top-down world events, economy, vendors, illicit markets, noble luxury, and draught goods.
...
- `MILESTONE_10_PERSISTENCE_SAVE_SCHEMA_MIGRATION_AND_BACKUPS.md` - world/player/profile save separation, schema versions, migrations, backups, autosaves, stale references, server authority, and persistence validation.
...
- Faction, population, economy, market, vendor, raw-material source, shipment, reinforcement import, deferred sector simulation, top-down world event, and probability-ledger editors belong primarily to Milestone 04.
...
- Save schema, migration, backup, autosave, stale-reference, world/player/profile separation, and persistence validation editors/audits belong primarily to Milestone 10.
...
- Quest editor.
- Scheme/Ages editor.
- Economy/vendor/provenance/world-event editor.
...
- Persistence/save-schema/migration audit surface.
- Mod API/example mod validation surface.

No editor is allowed to become a new uncontrolled design silo. Each editor must declare its owning milestone, data source, player-facing localization fields, validation rules, package/export behavior, save/persistence behavior where relevant, and audit outputs.
...
- Local districts may run operational ledgers for workers, vendors, rooms, supplies, schemes, contracts, vehicles, production, and world events.
- Distant zones and out-of-sector systems should defer into probability ledgers using faction strength, influence, wealth, route access, supply pressure, raw-material availability, reinforcement demand, event modifiers, and source provenance.

Out-of-sector systems should not pretend to fully simulate every mine, shipment, traveler, worker, or faction actor. They should produce auditable outcomes: shipments arrive or fail, reinforcements become available or expire, shortages worsen or ease, prices move, faction strength changes, faction influence changes, top-down events modify state, and schemes progress or stall.

When distant simulation creates a local effect, that effect should preserve provenance. Imported people, raw materials, weapons, food, medicine, luxury goods, draught goods, vehicles, and supplies should be able to say whether they came from local mining, local salvage, local production, outside-sector shipment, off-world import, train arrival, black-market route, faction reserve, event relief/seizure, or unresolved fallback.
...
- Population, provenance, vendors, markets, luxury, narcotics economy, draught trade, raw-material provenance, reinforcement imports, deferred simulation, top-down world events -> Milestone 04.
...
`LEGACY_MILESTONE_SOURCE_MAP.md` is the cleanup boundary for older topical milestone and supplement files. It maps each stale topical file to the ordered milestone that now owns its content.

Topical files should not be deleted casually. Treat them as legacy source/archive files until:

1. Their content has been absorbed into ordered milestones or runtime implementation.
2. The ordered milestone or `LEGACY_MILESTONE_SOURCE_MAP.md` names the source file it absorbed.
3. `MILESTONE_INDEX.md` references the ordered structure and source-map boundary.
4. The user explicitly approves archival, deprecation, movement, or deletion after verification.
...
Do not perform destructive cleanup without explicit user approval.
...
- Save/persistence ownership exists for any player-visible world or player state added by the milestone.
...
1. Verify `MILESTONE_INDEX.md` points to the ordered `00` through `10` sequence.
2. Verify `MASTER_DEVELOPMENT_PLAN.md` points to `MILESTONE_INDEX.md` rather than duplicating every milestone file.
3. Verify `LEGACY_MILESTONE_SOURCE_MAP.md` routes stale topical files to their canonical ordered homes.
4. Verify no new root-level or random `docs/` sprawl was introduced beyond the ordered milestone set and explicit source map.
5. Confirm the repo still treats the master plan, standards, governance, development history, milestone index, ordered milestones, and source map as durable authority/reference documents.

After that, a safe runtime handoff would likely begin with Milestone 02, Milestone 01, or Milestone 10 because UI/readability, asset audit, and persistence boundaries support later systems without prematurely expanding simulation complexity.

## Verification checklist

Before calling the ordered milestone consolidation complete, verify:

- Milestone 00 through 10 exist.
- `MILESTONE_INDEX.md` lists the ordered sequence.
- `LEGACY_MILESTONE_SOURCE_MAP.md` exists and maps old topical files to ordered homes.
...
- The editor suite has an owning milestone route for faction, room, entity, vehicle, object, item, feature, quest, worldgen, economy, simulation, and persistence tools.
- Deferred out-of-sector simulation routes through probability/provenance/faction-strength/event systems instead of full distant ticking.
- Persistence ownership routes through Milestone 10 for world/player/profile/launcher/cache/editor/content state.
...
This document does not require all persistence work to be implemented before gameplay development continues. It defines where save ownership, migration, backup, and validation rules are governed.

## Deferred checkpoint summary

The milestone system should now function as a phase-aligned implementation map. The master plan remains the authority. The ordered milestones provide depth. The index provides navigation. `LEGACY_MILESTONE_SOURCE_MAP.md` fences old topical files as source/archive material until explicitly cleaned up. Codex handoffs should be narrow, compile-safe, and tied to one ordered milestone at a time.

The editor suite should converge under owned milestone routes rather than isolated source sprawl. Faction, room, entity, vehicle, object, item, feature, quest, worldgen, economy, simulation, and persistence editors should be data-owned, validation-aware, localization-aware, publish-safe, save-aware, and auditable. Deferred out-of-sector simulation should collapse distant activity into probability/provenance/faction-strength/event ledgers while preserving source provenance for people, raw materials, shipments, and supplies that enter the playable sector.
