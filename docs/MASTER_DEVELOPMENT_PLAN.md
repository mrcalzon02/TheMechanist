...
- Phase 12.1 through Phase 12.10 need secondary expansion before construction systems are treated as faction-scale gameplay because player-built rooms, machines, vehicles, bases, defenses, and owned assets must raise heat/suspicion through a readable sliding scale rather than silently letting the player become a faction without consequence.
- Phase 17.1 through Phase 17.17 need secondary expansion before scheme quests are implemented because player-facing mission offers need planning/execution/cooldown timing, variable availability, non-synchronized activation times, minimum active duration, countdown messaging, active participation rules, neutral missed-quest standing behavior, clear objective highlighting, pointer arrows, zero-weight quest items, proof-of-death evidence, fail conditions, and compact NPC dialogue surfaces.
...
Goal: make player and faction spaces physically meaningful and make player expansion visible to rival powers.
...
- Phase 12.10 - Player asset heat and suspicion pass. Player construction, ownership, and accumulation of rooms, machines, vehicles, defenses, and production capacity should raise heat and suspicion by noticeable but variable amounts. A single living room, workshop, or purchased car should make the player more noticeable without necessarily making them a strategic threat. Owning multiple vehicles, many rooms, production machines, armed defenses, or faction-like infrastructure should raise heat substantially on a sliding scale, causing proper factions to notice competition, reassess the player, and potentially react through surveillance, offers, warnings, pressure, schemes, raids, taxes, extortion, recruitment, or diplomacy.

Exit criteria: buildings and rooms govern access, cover, raids, ownership, later combat, and faction notice. The player can build a modest life without instantly becoming a warlord, but sustained expansion into rooms, machines, vehicles, production, and defenses makes the player increasingly visible as a rising faction-scale actor.
...
Goal: convert faction presence, leadership schemes, supply needs, strategic plans, player expansion, and quest evidence into player-facing work, risk, trade, reputation, heat, and visible participation opportunities.
...
- Phase 17.16 - Quest-critical item and proof-of-death evidence pass. All quest items that the player is required to pick up for quest performance should have zero weight so quest progress is not blocked by inventory burden. Entities required to be killed for a quest should produce an identifiable proof-of-death item in their inventory or corpse container when they die. The system should not care who killed the target; it should care that the target died and that the death-proof item was produced on the body for player pickup, recovery, delivery, or confirmation.
- Phase 17.17 - Player expansion heat and faction competition bridge pass. Player-owned rooms, machines, vehicles, production facilities, defenses, and other faction-like assets should feed heat, suspicion, and competitive attention into faction reputation/economy systems. The scale should be sliding and contextual: one room and one car may be noticeable but modest, while three cars, a dozen rooms, production machines, armed defenses, or a private vehicle fleet should sharply increase attention, competition, and scheme pressure from existing factions.

Exit criteria: quests and economy produce work tied to faction facilities, population roles, item provenance, vehicle provenance, actual supply needs, active leadership plans, scheme lifecycle timing, reliable quest evidence, and player expansion pressure. The player can see when scheme-related missions are being planned, when they can join an active operation, why an opportunity is unavailable, when a repeat offer is expected to return, where to go next, which object or actor matters once they arrive, what evidence proves completion, and why factions begin to care when the player expands like a rival power.
...
- Phase 18.17 - Quest evidence and expansion heat editor/audit pass. Let future data/editor surfaces inspect zero-weight quest item flags, proof-of-death evidence definitions, corpse-spawn behavior, evidence labels, player asset heat values, suspicion curves, faction attention thresholds, and reaction hooks without exposing raw internal IDs in normal player UI.

Exit criteria: content can be added through data/editor surfaces instead of central-code sprawl, and asset promotion, population provenance, item provenance, vehicle provenance, ages-of-control histories, scheme quests, quest evidence, and player expansion heat can be inspected or extended through owned data surfaces.
...
- Phase 19.19 - Quest evidence and player expansion heat release audit. Verify zero-weight quest-critical items, proof-of-death evidence spawning on killed targets regardless of killer, corpse/container pickup behavior, evidence labels, player construction heat, vehicle/room/machine ownership heat, suspicion scaling, faction attention thresholds, and release-safe player-facing explanations before release-candidate claims.

Exit criteria: development can proceed phase by phase without losing context, duplicating authorities, inventing fake implementation, shipping unclear assets, or claiming provenance/history/scheme systems that do not yet have population, workforce, production, vehicle, room-control, active-scheme visibility, leadership-scheme context, player-facing mission lifecycle behavior, clear objective guidance, reliable quest evidence, and faction response to player expansion.
...
### Quest-critical items and proof-of-death evidence

Quest-critical pickup items required for quest performance should have zero weight. A quest should not become impossible because the player cannot carry the one mandatory document, token, component, sample, or proof item the mission requires. Kill-target quests should not require the player to personally land the killing blow unless the quest explicitly says so. When a required target dies, the target's corpse or death container should produce an identifiable proof-of-death item for pickup. The system should validate that the target died and that the evidence exists or was recovered, not obsess over who or what killed the target.

### Player expansion heat and faction attention

Player-owned construction and asset accumulation should feed a sliding heat and suspicion model. A single room, modest shelter, small workshop, or ordinary car should make the player visible but not automatically dangerous. Multiple rooms, multiple vehicles, production machines, armed defenses, restricted machinery, vehicle fleets, or faction-like infrastructure should spike heat and suspicion in noticeable, variable amounts. The more the player expands into a proper faction, the more proper factions should take note, raise competition factors, and respond through offers, warnings, surveillance, extortion, raids, recruitment attempts, diplomacy, legal pressure, sabotage, or leadership schemes.
