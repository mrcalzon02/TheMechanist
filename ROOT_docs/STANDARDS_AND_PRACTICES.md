...
- NPC pathing in real-time or continuous-tick modes must be buffered, budgeted, or coalesced through an owned scheduler so stacked NPCs do not issue simultaneous expensive path requests. Immediate player-near movement may receive priority, but distant or repeated path requests should defer into cached intent, short local steering, or abstract simulation debt.
...
The repository includes `ROOT_tools/build/verify_java17_classfiles.ps1` as a release-gate helper. This helper is not optional decoration: when packaging through a manual/non-Maven path, run it against `TheMechanist.jar`, `TheMechanistServer.jar`, and any loose class output that will ship. If the helper reports any classfile above major 61, stop the release and rebuild with the correct target before producing the zip.
