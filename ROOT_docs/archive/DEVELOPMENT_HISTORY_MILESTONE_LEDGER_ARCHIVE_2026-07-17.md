...
## Milestone 05 - Live Room-Editor Construction Parity Inspection Slice

Advanced Phase 18.1 with a live, read-only construction-parity snapshot in the existing Room Editor. Every room in the active world's room ledger receives one stable record keyed by world location and room index. Each record exposes live location and geometry, room/profile presence, special-room state, room name and source, the exact profile-declared faction separately from the exact current controller, their normalized strategic families, and both exact-identity and family-level divergence. Blueprint state is classified as `MAPPED`, `NON_ACQUIRABLE_EXCEPTION`, or `UNMAPPED_GAP`; mapped rooms name their exact plan, while gaps use the explicit `No exact blueprint` sentinel and retain their acquisition, faction-use, and exception explanation. A single audit row counts all mapping states, both controller-divergence levels, special rooms, and missing profiles.

Refresh replaces prior runtime rows without duplication, preserves stable IDs, observes subsequent live room and controller changes, and leaves edited snapshot values isolated in the mod overlay rather than applying them to the world. The in-game Room Editor exposes the same `Refresh Live World` route. Its boundary explicitly does not prove plan ownership, vendor stock, price, installed equipment, or apply editor changes to the live world. The room mapper now uses ordered, word-bounded matching so short fragments such as `lab` cannot match `labor`; a newly observed Basic Forge Annex resolves deterministically to the EMM Micro Forge plan. Gate integration also hardened faction-market scarcity: structured essential-stock identity keeps real emergency rations open, while explicit construction IDs and term boundaries prevent water-named blueprints or calibration tools from inheriting relief exemptions.

Verification: Java 17 full-tree compile passed; focused room-construction parity editor, room-construction audit, room-control provenance, and faction room-shell construction smokes passed; expanded Gate 3 passed including the new Room Editor coverage alongside the existing generated-world save, topology, and production-input warnings. The client package rebuilt from 807 Java sources with 1947 class files and 6096 package assets; the Java 17 package scan passed for 3894 classfiles across the unfolded package and JAR, highest major version 61; the new Infopedia, Economy parity, Room parity, room-control provenance, and adjacent strategic-asset smokes passed from packaged classes; package boot stayed alive for 8 seconds; function and Mermaid maps refreshed at 807 Java modules and 0 unpositioned modules; repository manifest refreshed with 224321 indexed rows and zero hash errors.

## Milestone 05 - Economy-Editor Construction Recipe Parity Inspection Slice

Advanced a bounded Phase 18.2 definition-and-audit slice by adding the complete registered construction-recipe catalog to the existing Economy Editor. Each `BuildRecipe` receives one refreshable `ConstructionParityInspection` row plus one aggregate audit. Rows expose category, player and faction capability, public-plan or licensed-blueprint identity, mapping validity, issuing source, vendor category, acquisition and access rules, legal class, material and workforce summaries, and explicit parity exception class and reason. Blueprint validation now requires a stable ID round trip to the same recipe and, for licensed plans, a real catalog folio.

Public recipes truthfully report default availability through the public construction catalog with no invented seller, grantor, or vendor category. Licensed plans retain their issuing faction and representative channel; the EMM Micro Forge remains conditional for the player and supported through live faction construction, while public recipes such as the Storage Crate retain their documented faction-side exception. Refresh replaces old runtime rows, preserves user-authored records, and keeps edits mod-local. The records and editor UI explicitly identify this as a static, refreshable catalog inspection with `liveWorldDependent=false`; it is not live ownership, vendor inventory, facility, current price, provenance, or heat state. Refresh status now reports live-world and catalog/definition counts separately instead of labeling every Economy Editor record as live.

Verification: Java 17 full-tree compile passed; focused construction-parity inspection and Economy Editor parity smokes plus the adjacent economy/world-editor Infopedia smoke passed; expanded Gate 3 passed including the new construction-parity editor coverage alongside the existing generated-world save, topology, and production-input warnings. Packaged Economy parity coverage is consolidated with the live Room Editor slice above.

## Milestone 05 - First-Class Construction Blueprint Infopedia Dossier Slice

Completed the Phase 17.4 registered-construction bridge by giving every `BuildRecipe` a first-class searchable `CONSTRUCTION - <name> [category]` Infopedia row. Construction recipe rows and licensed blueprint item rows cross-link to the same canonical dossier instead of depending on generic item-market prose. Each dossier answers player and faction capability, public-catalog or licensed-plan access, issuing source and ordinary acquisition channel, reputation or license gates, exact requirements and materials, construction effort, workforce, legality and attention, and the current parity exception.

Public plans now state that they are available by default from the public construction catalog and require neither a licensed folio nor an invented market, contract, or salvage unlock. Licensed blueprint-item details bypass contradictory generic seller text and retain their actual faction, representative, contract, recovery, theft, or counterfeit pathways. Legality derives from structural recipe metadata rather than flavor prose, preventing unrelated words such as `stolen` from changing a public plan's legal class, while revealed contract leads remain distinct from ownership.

Verification: Java 17 full-tree compile passed; focused blueprint-acquisition path, blueprint Infopedia, construction-parity inspection, and room-control provenance smokes passed; expanded Gate 3 passed including the new full-catalog Infopedia coverage alongside the existing generated-world save, topology, and production-input warnings. Packaged Infopedia coverage is consolidated with the live Room Editor slice above.
