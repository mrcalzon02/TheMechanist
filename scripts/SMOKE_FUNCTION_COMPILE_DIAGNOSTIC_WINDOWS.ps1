...
$runRoot = Join-Path $diagRoot "function_compile_smoke_$stamp"
...
if ($compileExit -eq 0) {
    Write-Host "COMPILE DIAGNOSTIC PASS: compile succeeded. Logs: $runRoot"
} else {
    Write-Host "COMPILE DIAGNOSTIC FAIL: compile failed with exit $compileExit. Logs: $runRoot"
}
