...
final class SemanticRenderAssetResolver {
    ...
    static boolean canUse(AssetMetadata asset, RenderIntent intent) {
        ...
        return switch (intent) {
            ...
            case MARKET_COUNTER -> furnitureType(asset) && themed(haystack, "market counter", "shop counter", "stall counter", "trader counter");
            case ADMINISTRATIVE_DESK -> furnitureType(asset) && themed(haystack, "administrative desk", "office desk", "records desk");
            case INTERROGATION_DESK -> furnitureType(asset) && themed(haystack, "interrogation desk", "security interview desk");
            ...
            case MEDICAL_ITEM_ICON -> itemIcon(asset) && themed(haystack,
                    "medkit", "medical kit", "bandage", "suture", "medicine", "antiseptic",
            ...
            case RELIGIOUS_OBJECT_ITEM_ICON -> itemIcon(asset) && themed(haystack,
                    "relic", "prayer", "holy object", "devotional",
    ...
    private static int priority(AssetMetadata asset, RenderIntent intent) {
        ...
        if (intent == RenderIntent.INDUSTRIAL_FLOOR) {
            if (contains(h, "industrial", "factory")) score += 10;
            else if (contains(h, "machine shop", "workshop")) score += 4;
        }
        if (intent == RenderIntent.INDUSTRIAL_WALL) {
            if (contains(h, "industrial", "factory")) score += 10;
            else if (contains(h, "machine shop", "workshop")) score += 4;
        }
        ...
        if (intent == RenderIntent.MEDICAL_FLOOR) {
            if (contains(h, "medical", "clinic", "hospital")) score += 10;
            else if (contains(h, "surgery")) score += 4;
        }
        if (intent == RenderIntent.SECURITY_FLOOR) {
            if (contains(h, "security", "checkpoint")) score += 10;
            else if (contains(h, "prison", "brig")) score += 4;
        }
        ...
        if (intent == RenderIntent.NOBLE_FLOOR) {
            if (contains(h, "noble", "estate", "manor")) score += 10;
            else if (contains(h, "luxury")) score += 4;
        }
        if (intent == RenderIntent.SLUM_FLOOR) {
        ...
        if (intent == RenderIntent.TRAFFIC_LIGHT_FIXTURE) {
            if (contains(h, "traffic light")) score += 10;
            else if (contains(h, "signal light", "crossing signal")) score += 4;
        }
        if (intent == RenderIntent.GENERATOR_MACHINE) {
            if (contains(h, "generator", "power generator")) score += 10;
            else if (contains(h, "genset")) score += 4;
        }
        if (intent == RenderIntent.TRANSFORMER_MACHINE) {
            if (contains(h, "power transformer", "electrical transformer")) score += 10;
            else if (contains(h, "transformer")) score += 4;
        }
        ...
        if (intent == RenderIntent.SECURITY_CAMERA_FIXTURE) {
        ...
        if (intent == RenderIntent.MEDICAL_TABLE) {
            if (contains(h, "medical table")) score += 10;
            else if (contains(h, "operating table", "surgery table")) score += 4;
        }
        ...
        if (intent == RenderIntent.ADMINISTRATIVE_DESK) {
            if (contains(h, "administrative desk")) score += 10;
            else if (contains(h, "office desk", "records desk")) score += 4;
        }
        if (intent == RenderIntent.INTERROGATION_DESK) {
            if (contains(h, "interrogation desk")) score += 10;
            else if (contains(h, "security interview desk")) score += 4;
        }
        if (intent == RenderIntent.TOOLBOX_CONTAINER && contains(h, "toolbox", "tool box")) score += 10;
        if (intent == RenderIntent.MEDICAL_CABINET_CONTAINER) {
            if (contains(h, "medical cabinet")) score += 10;
            else if (contains(h, "medicine cabinet", "clinic cabinet")) score += 4;
        }
        if (intent == RenderIntent.WEAPONS_LOCKER_CONTAINER) {
        ...
        if (intent == RenderIntent.CARGO_CONTAINER) {
            if (contains(h, "cargo container")) score += 10;
            else if (contains(h, "shipping container", "cargo crate", "freight crate")) score += 4;
        }
        if (intent == RenderIntent.FILING_CABINET_CONTAINER) {
        ...
        if (intent == RenderIntent.WEAPON_ITEM_ICON) {
            if (contains(h, "knife", "knives", "shiv", "dagger", "sword", "axe", "hatchet", "spear", "polearm", "pistol", "rifle", "carbine", "shotgun", "bolter", "flamer", "melta", "stubber", "autocannon", "lasgun", "lascannon")) score += 10;
            else if (contains(h, "gun", "blade", "ammo", "ammunition")) score += 4;
        }
        if (intent == RenderIntent.ARMOR_ITEM_ICON) {
            if (contains(h, "armor", "armour", "helmet", "helm", "vest", "carapace", "flak")) score += 10;
            else if (contains(h, "clothing", "coat", "robe", "uniform", "rags", "coverall", "workwear", "overalls")) score += 4;
        }
        ...
        if (intent == RenderIntent.MEDICAL_ITEM_ICON) {
            if (contains(h, "medkit", "medical kit", "bandage", "suture", "antiseptic", "tourniquet", "splint", "first aid", "trauma kit")) score += 10;
            else if (contains(h, "medicine")) score += 4;
        }
        if (intent == RenderIntent.DRUG_ITEM_ICON) {
            if (contains(h, "stimulant", "injector", "opiate", "sedative", "combat stim", "painkiller")) score += 10;
        ...
        if (intent == RenderIntent.INDUSTRIAL_COMPONENT_ITEM_ICON) {
            if (contains(h, "machine part", "industrial part", "bearing", "fastener", "rivet", "circuit", "scrap plate")) score += 10;
            else if (contains(h, "component", "construction supplies", "reagent")) score += 4;
        }
        if (intent == RenderIntent.TRADE_GOOD_ITEM_ICON) {
            if (contains(h, "trade good", "luxury good", "cargo lot", "export good", "import good")) score += 10;
            else if (contains(h, "commodity", "barter", "merchandise", "wares")) score += 4;
        }
    ...
    private static boolean machineType(AssetMetadata asset) { return asset.type() == AssetType.MACHINE || asset.type() == AssetType.OBJECT; }
    ...
    private static boolean armorIcon(AssetMetadata asset) { return asset.type() == AssetType.ARMOR_ICON || itemIcon(asset); }
