...
class Persistence {
    ...
    static void readCore(GamePanel g, Properties p) {
        ...
        g.turn = getInt(p,"run.turn",0); g.worldTurn = getLong(p,"run.worldTurn", g.turn); g.playerX = clamp(getInt(p,"run.playerX", g.world.startPoint().x),0,g.world.w-1); g.playerY = clamp(getInt(p,"run.playerY", g.world.startPoint().y),0,g.world.h-1);
        ...
        g.baseObjects.clear(); for(String s: decList(p.getProperty("base.objects",""))) { String[] a=s.split("\\|",24); if(a.length>=5) try{ BaseObject b=new BaseObject(a[0], a[1].isEmpty()?'?':a[1].charAt(0), Integer.parseInt(a[2]), Integer.parseInt(a[3]), 0, 0); b.capacity=Integer.parseInt(a[4]); if(a.length>=6) b.qualityName=a[5]; if(a.length>=7) try{ b.faction=Faction.valueOf(a[6]); }catch(Exception ignored){} if(a.length>=8) b.charges=Integer.parseInt(a[7]); if(a.length>=9) b.integrity=Integer.parseInt(a[8]); if(a.length>=10) b.assignedRecipe=a[9]; if(a.length>=11) b.assignedWorker=a[10]; if(a.length>=12) b.businessOpen=Boolean.parseBoolean(a[11]); if(a.length>=13) b.permittedBusiness=Boolean.parseBoolean(a[12]); if(a.length>=14) try{ b.businessHeat=Integer.parseInt(a[13]); }catch(Exception ignored){} if(a.length>=15) try{ b.productionQueueTarget=Integer.parseInt(a[14]); }catch(Exception ignored){} if(a.length>=16) try{ b.productionQueueRemaining=Integer.parseInt(a[15]); }catch(Exception ignored){} if(a.length>=17) b.underConstruction=Boolean.parseBoolean(a[16]); if(a.length>=18 && !a[17].isBlank()) b.finalSymbol=a[17].charAt(0); if(a.length>=19) b.constructionRequiredItems=a[18]; if(a.length>=20) b.constructionInsertedItems=a[19]; if(a.length>=21) try{ b.constructionLaborRequired=Integer.parseInt(a[20]); }catch(Exception ignored){} if(a.length>=22) try{ b.constructionLaborDone=Integer.parseInt(a[21]); }catch(Exception ignored){} if(a.length>=23) try{ b.constructionVisualProgress=Integer.parseInt(a[22]); }catch(Exception ignored){} if(a.length>=24) b.machineKnowledge=a[23]; if(!b.underConstruction) g.configureBaseObject(b); g.baseObjects.add(b); if(g.world.inBounds(b.x,b.y)) g.world.tiles[b.x][b.y]=b.symbol; }catch(Exception ignored){} }
