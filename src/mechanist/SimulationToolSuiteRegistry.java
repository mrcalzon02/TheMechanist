...
final class SimulationToolSuiteRegistry {
    ...
    static final String POPULATION_EDITOR = "Population Editor";
    static final String ECONOMY_EDITOR = "Economy Editor";
    static final String REINFORCEMENT_EDITOR = "Reinforcement Editor";
    static final String DEFERRED_NETWORK_EDITOR = "Deferred Network Editor";
    static final String WORLD_EVENT_EDITOR = "World Event Editor";
    ...
    private static final List<ToolSpec> SPECS = List.of(
            ...
            new ToolSpec(ROOM_EDITOR, true, 30, "room bracket", "legacy room editor tab plus live parity and purpose inspection", "Room footprint, layout, current control, explicit declared purpose, physical requirements, operating status, type-specific capacity, blockers, evidence, reinforcement-source eligibility, and room-to-blueprint parity records.", "dimensions,oxygen seal,terminal,layout palette,placement nodes,current controller,declared profile faction,declared room purpose,requirement status,physical evidence,type-specific operating capacity,child-care capacity,barracks duty and muster capacity,blockers,room-to-blueprint mapping,explicit parity exceptions,live world refresh,mod export"),
            ...
            new ToolSpec(POPULATION_EDITOR, false, 41, "population bracket", "population provenance and demand ledgers", "Population origins, capacity, workforce, care, demand, and living-condition records.", "origin,capacity,workforce,facilities,care,happiness,demand,live world refresh,mod export"),
            new ToolSpec(ECONOMY_EDITOR, false, 42, "economy bracket", "faction market, provenance, and construction-parity ledgers", "Finite reserves, provenance, legality, market policy, shipment, scarcity, contract hooks, and refreshable construction-catalog parity records.", "reserves,provenance,legality,vendors,shipments,scarcity,contracts,blueprints,construction catalog capability,parity exceptions,live world refresh,mod export"),
            new ToolSpec(REINFORCEMENT_EDITOR, false, 43, "reinforcement bracket", "personnel replacement and import ledgers", "Reinforcement sources, prices, timing, expiry, capacity, and import-node records.", "sources,costs,cooldowns,expiry,capacity,import nodes,live world refresh,mod export"),
            new ToolSpec(DEFERRED_NETWORK_EDITOR, false, 44, "deferred network bracket", "distant faction probability ledgers", "Distant faction strength, supply, route, probability, timing, and outcome records.", "strength,influence,wealth,suppliers,routes,probability,outcomes,live world refresh,mod export"),
            new ToolSpec(WORLD_EVENT_EDITOR, false, 45, "world event bracket", "top-down event and recovery ledgers", "World-event eligibility, duration, effects, notices, facility mutation, restrictions, and recovery records.", "families,eligibility,duration,severity,economy,population,routes,facilities,news,recovery,live world refresh,mod export"),
            new ToolSpec(QUEST_EDITOR, true, 46, "quest bracket", "legacy quest editor pointer / Milestone 07 quest editor doctrine", "Quest identity, localization, lifecycle, objectives, evidence, rewards, consequences, guidance, and validation records.", "quest id,localization,lifecycle,objectives,evidence,rewards,consequences,objective guidance,validation,preview,mod export"),
    ...
    static LinkedHashMap<String, Object> defaultPropertiesFor(String editorName) {
        return switch (editorName) {
            ...
            case POPULATION_EDITOR -> withToolMetadata(editorName, props("originMode", "arcology-born", "faction", "HIVER", "roomName", "Hab block", "sourceKind", "local roster", "capacity", 12, "available", 4, "assigned", 8, "dead", 0, "workforceProfile", "civilian services", "demandProfile", "food,water,bed,pay", "facilityId", "", "careProviders", 0, "happiness", 60));
            case ECONOMY_EDITOR -> withToolMetadata(editorName, props("ledgerType", "essential reserve", "faction", "HIVER", "itemName", "Emergency rations", "sourceKind", "local production", "sourceLabel", "Hab kitchen", "sourceFacilityId", "", "route", "facility -> local vendor", "legality", "ordinary legal stock", "vendorCategory", "provisions", "marketPreference", "essential", "contractHook", "depleted reserve delivery", "capacity", 6, "remaining", 6, "restockIntervalTurns", 1008, "internalUse", true, "externalSale", true, "vaultOnly", false));
            case REINFORCEMENT_EDITOR -> withToolMetadata(editorName, props("sourceMode", "train-import", "faction", "HIVER", "scriptCost", 0, "minimumDelayTurns", 150, "delaySpreadTurns", 101, "arrivalWindowTurns", 48, "prerequisite", "faction-linked rail intake", "requestedTurn", 0, "dueTurn", 150, "expiresTurn", 198, "capacityRequired", 1, "importNode", "sector exchange"));
            case DEFERRED_NETWORK_EDITOR -> withToolMetadata(editorName, props("faction", "HIVER", "sourceRegion", "adjacent spire districts", "strength", 50, "influence", 50, "wealth", 50, "populationPressure", 25, "reinforcementDemand", 0, "supplierReliability", 50, "routeSafety", 50, "shipmentPressure", 0, "rawMaterialAvailability", 50, "importExportCapacity", 40, "eventPressure", 0, "resolutionIntervalTurns", 36, "lastChancePct", 0, "lastRoll", 0, "lastOutcome", "No outcome yet."));
            case WORLD_EVENT_EDITOR -> withToolMetadata(editorName, props("eventType", "SUPPLY_SHOCK", "title", "External supply shock", "status", "SCHEDULED", "scope", "sector", "targetFaction", "NONE", "marketCategory", "critical supplies", "durationTurns", 1008, "severity", 3, "eligibilitySummary", "external supplier disruption", "economySupplyPct", -16, "populationPressureDelta", 5, "shipmentDelayTurns", 48, "shipmentRiskDelta", 15, "reinforcementDelayTurns", 30, "deferredPressureDelta", 20, "importClosed", false, "exportClosed", false, "offMapSalesClosed", false, "roomMutationHook", "shortage-control receiving room", "newsExposure", "market notices and broadcasts", "vendorRestriction", "outside restocking slows", "vendorException", "local production remains available", "aftermath", "supplier cadence recovers"));
    ...
    static List<SimulationEditorRepository.EditableEntity> seedEntitiesFor(String editorName) {
        List<SimulationEditorRepository.EditableEntity> raw = switch (editorName) {
            ...
            case POPULATION_EDITOR -> entities(
                    entity("population-hab-roster", "Hab Resident Roster", props("originMode", "arcology-born", "faction", "HIVER", "roomName", "Hab Cell Block", "sourceKind", "resident roster", "capacity", 24, "available", 6, "assigned", 18, "dead", 0, "workforceProfile", "residents and civilian services", "demandProfile", "food,water,bed,pay,medicine", "facilityId", "hab-block", "careProviders", 0, "happiness", 60)),
                    entity("population-creche-roster", "Creche Cohort", props("originMode", "faction births and wards", "faction", "HIVER", "roomName", "Faction Creche", "sourceKind", "immature cohort", "capacity", 12, "available", 0, "assigned", 8, "dead", 0, "workforceProfile", "one care provider per twelve children", "demandProfile", "double food,water,pediatric care,teaching", "facilityId", "creche", "careProviders", 1, "happiness", 75))
            );
            case ECONOMY_EDITOR -> entities(
                    entity("economy-essential-reserve", "Essential Food Reserve", props("ledgerType", "essential reserve", "faction", "HIVER", "itemName", "Emergency rations", "sourceKind", "local production", "sourceLabel", "Faction kitchen", "sourceFacilityId", "kitchen", "route", "kitchen -> provisioner", "legality", "ordinary legal stock", "vendorCategory", "provisions", "marketPreference", "essential", "contractHook", "depleted reserve delivery", "capacity", 8, "remaining", 8, "restockIntervalTurns", 1008, "internalUse", true, "externalSale", true, "vaultOnly", false)),
                    entity("economy-external-shipment", "External Material Shipment", props("ledgerType", "shipment", "faction", "MECHANIST_COLLEGIA", "itemName", "Machine part", "sourceKind", "train import", "sourceLabel", "outside-sector supplier", "sourceFacilityId", "rail-intake", "route", "supplier -> rail cargo -> forge", "legality", "regulated freight", "vendorCategory", "industrial", "marketPreference", "production input", "contractHook", "delayed shipment recovery", "capacity", 6, "remaining", 6, "restockIntervalTurns", 1512, "internalUse", true, "externalSale", true, "vaultOnly", false))
            );
            case REINFORCEMENT_EDITOR -> entities(
                    entity("reinforcement-train", "Reinforcement Train", props("sourceMode", "train-import", "faction", "HIVER", "scriptCost", 0, "minimumDelayTurns", 150, "delaySpreadTurns", 101, "arrivalWindowTurns", 48, "prerequisite", "faction-linked rail intake", "requestedTurn", 0, "dueTurn", 150, "expiresTurn", 198, "capacityRequired", 1, "importNode", "sector exchange")),
                    entity("reinforcement-paid-local", "Paid Local Recruitment", props("sourceMode", "paid-local", "faction", "HIVER", "scriptCost", 24, "minimumDelayTurns", 65, "delaySpreadTurns", 46, "arrivalWindowTurns", 36, "prerequisite", "available local population roster", "requestedTurn", 0, "dueTurn", 65, "expiresTurn", 101, "capacityRequired", 1, "importNode", "local roster"))
            );
            case DEFERRED_NETWORK_EDITOR -> entities(
                    entity("deferred-network-standard", "Distant Faction Network", props("faction", "HIVER", "sourceRegion", "adjacent spire districts", "strength", 50, "influence", 50, "wealth", 50, "populationPressure", 25, "reinforcementDemand", 0, "supplierReliability", 50, "routeSafety", 50, "shipmentPressure", 0, "rawMaterialAvailability", 50, "importExportCapacity", 40, "eventPressure", 0, "resolutionIntervalTurns", 36, "lastChancePct", 0, "lastRoll", 0, "lastOutcome", "No outcome yet."))
            );
            case WORLD_EVENT_EDITOR -> entities(
                    entity("world-event-supply-shock", "External Supply Shock", props("eventType", "SUPPLY_SHOCK", "title", "External supply shock", "status", "SCHEDULED", "scope", "sector", "targetFaction", "NONE", "marketCategory", "raw materials and critical supplies", "durationTurns", 1008, "severity", 3, "eligibilitySummary", "external supplier disruption", "economySupplyPct", -16, "populationPressureDelta", 5, "shipmentDelayTurns", 48, "shipmentRiskDelta", 15, "reinforcementDelayTurns", 30, "deferredPressureDelta", 20, "importClosed", false, "exportClosed", false, "offMapSalesClosed", false, "roomMutationHook", "shortage-control receiving room", "newsExposure", "market notices and broadcasts", "vendorRestriction", "outside restocking slows", "vendorException", "local production remains available", "aftermath", "supplier cadence recovers")),
                    entity("world-event-relief", "Relief Shipment", props("eventType", "RELIEF_SHIPMENT", "title", "Relief shipment", "status", "SCHEDULED", "scope", "sector", "targetFaction", "NONE", "marketCategory", "food, water, and medicine", "durationTurns", 1008, "severity", 2, "eligibilitySummary", "shortage, casualties, or displacement", "economySupplyPct", 14, "populationPressureDelta", -10, "shipmentDelayTurns", -36, "shipmentRiskDelta", -8, "reinforcementDelayTurns", -24, "deferredPressureDelta", -12, "importClosed", false, "exportClosed", false, "offMapSalesClosed", false, "roomMutationHook", "temporary relief intake", "newsExposure", "civic notices, newspapers, and broadcasts", "vendorRestriction", "relief stock is rationed", "vendorException", "ordinary internal vendors remain open", "aftermath", "relief intake closes after reserves stabilize"))
            );
    ...
    static List<SimulationEditorRepository.EntityRef> defaultSelectedRefs() {
        return List.of(
                ...
                new SimulationEditorRepository.EntityRef(POPULATION_EDITOR, "population-hab-roster"),
                new SimulationEditorRepository.EntityRef(ECONOMY_EDITOR, "economy-essential-reserve"),
                new SimulationEditorRepository.EntityRef(REINFORCEMENT_EDITOR, "reinforcement-train"),
                new SimulationEditorRepository.EntityRef(DEFERRED_NETWORK_EDITOR, "deferred-network-standard"),
                new SimulationEditorRepository.EntityRef(WORLD_EVENT_EDITOR, "world-event-supply-shock"),
    ...
    static Map<String, List<String>> linkOptionsFor(String editorName, SimulationEditorRepository repository) {
        return switch (editorName) {
            ...
            case POPULATION_EDITOR -> LinkCatalog.populationLinks(repository);
            case ECONOMY_EDITOR -> LinkCatalog.economyLinks(repository);
            case REINFORCEMENT_EDITOR -> LinkCatalog.reinforcementLinks(repository);
            case DEFERRED_NETWORK_EDITOR -> LinkCatalog.deferredNetworkLinks(repository);
            case WORLD_EVENT_EDITOR -> LinkCatalog.worldEventLinks(repository);
